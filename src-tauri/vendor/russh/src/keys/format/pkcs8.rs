...
pub fn encode_pkcs8_encrypted(
    ...
    let doc = pvi.encrypt_with_params(
        // 本地补丁（vendor/russh）：适配 pkcs5 0.8.0 stable —— rc 时代名为
        // `pbkdf2_sha256_aes256cbc`，stable 改名为 `generate_pbkdf2_sha256_aes256cbc`
        // （签名相同，均返回 Result）。项目依赖图（ironrdp→picky 锁 RustCrypto rc）
        // 把 pkcs5 抬升到 stable 0.8.0，russh 0.60.1 宽松声明被污染导致编译失败。
